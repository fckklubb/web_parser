...
def getXtraInfo(res_list: List[CFR], session: requests.Session) -> List[CFR]:
    for c in res_list:
...
def getOneColumnRates_almak(res_list: List[CFR], pick_up: datetime.date, days: int, session: requests.Session, *args, **kwargs) -> List[CFR]:
    __response = session.get(url=url)
    print(f"{url[:60]}.. Status code: ", __response.status_code, "..")
    ...
    if xtra:
        res_list = getXtraInfo(res_list, session)
    return res_list